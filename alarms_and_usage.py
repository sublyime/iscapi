...
for inst in list(api.get_response_json()):
    ...
    if r.status_code == 200:
        ...
        if total_alarms > 0:
            ...
            break         
# get usage for the same time period
# r = api.list_equipment_usage(instSn, params={"q": '{{"beginTime": "{0}"}}'.format(api.date_range(starttime,endtime))})
# api.display(r)
# usage_seconds = 0
# if r.status_code == 200:
#     for obj in r.json():
#         u = api.get_usage(obj.get('id'))
#         api.display(u)
#         if u.status_code == 200:
#             usage_seconds += u.json().get('duration')

# print("INFO: Found {0} alarms for {1} seconds of usage (SN={2})".format(total_alarms, usage_seconds, instSn))

# list all H2S alarms in the account for specified time period, with a peak exceeding 20ppm
# r = api.list_alarms(params={"q": '{{"time": "{0}", "peakReading": ">20", "gasCode": "G0002"}}'.format(timeframe), 'format': 'full' } )
# api.display(r)
# print("INFO: Found {0} H2S alarms exceeding 20ppm".format(len(r.json())) )
